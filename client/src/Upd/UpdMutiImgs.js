...
class UpdMutiImgs extends Component {
	constructor(props) {
		super(props);
	...
	find_imgs () {
		const {coll, fld, _docId, _mutiId} = this.props;
		Axios.post('/upd_mutiblob/find_imgs', {coll:coll, fld:fld, _docId:_docId, _mutiId:_mutiId}).then((resp) => {
	...
	ins_img () {
		...
		formData.append('_mutiId', _mutiId);
		formData.append('file', file);
		formData.append('scale', scale);

		Axios.post('/upd_mutiblob/ins_img', formData).then((resp) => {
			this.setState({imgs:resp.data});
		});
	}
	...
	del_img (img, idx) {
		const {coll, fld, _docId, _mutiId} = this.props;
		Axios.post('/upd_mutiblob/del_img', {coll:coll, fld:fld, _docId:_docId, _mutiId:_mutiId, _imgId:img._id, filename:img.filename}).then((resp) => {
			this.setState({imgs:resp.data});
		});
	}
	...
	upd_seq (_imgId, mov) {
		const {coll, fld, _docId, _mutiId} = this.props;
		Axios.post('/upd_mutiblob/upd_seq', {coll:coll, fld:fld, _docId:_docId, _mutiId:_mutiId, _imgId:_imgId, mov:mov}).then((resp) => {
				...
				<div className="updImgsItems">
					{this.state.imgs.map((img, idx) => (
						<div className="updImgsItem" key={idx}>
							<img className="updImgsView" src={img.url +'_XS.jpg'} />
